...
public class ThongKeForm extends javax.swing.JInternalFrame {
    ...
    public ThongKeForm() {   	
     ...
     // Fetch data and create charts
        ...
        txtQuantityProduct.setText(Integer.toString(MayTinhImpl.getInstance().getSl()));
        txtQuantityNcc.setText(Integer.toString(NhaCungCapImpl.getInstance().selectAll().size()));
        txtQuantityUser.setText(Integer.toString(AccountImpl.getInstance().selectAll().size()));
        //
        initTable();
        loadDataToTable();
        changeTextFind();
        //
        loadDataToTableAcc(AccountImpl.getInstance().selectAll());
        ...
        //
        loadDataToTableThongKeProduct(ThongKeImpl.getInstance().getThongKe());
    }
    ...
    private void loadDataToTable() {
        try {
            ArrayList<PhieuNhap> allPhieu = PhieuNhapImpl.getInstance().selectAll();
    ...
    public ArrayList<Phieu> searchTatCa(String text) {
        ...
        ArrayList<Phieu> armt = PhieuNhapImpl.getInstance().selectAllP();
        for (var phieu : armt) {
    ...
    public ArrayList<Phieu> searchPhieuXuat(String text) {
        ArrayList<Phieu> result = new ArrayList<>();
        ArrayList<PhieuXuat> armt = PhieuXuatImpl.getInstance().selectAll();
        for (var phieu : armt) {
            if (phieu.getMaPhieu().toLowerCase().contains(text.toLowerCase())
                    || phieu.getNguoiTao().toLowerCase().contains(text.toLowerCase())) {
                result.add(phieu);
    ...
    private void createChart1() {
        // Lấy instance của PhieuNhapImpl và dữ liệu tổng tiền theo nhà cung cấp
        ...
        // Tạo biểu đồ tròn với dataset đã tạo
        JFreeChart chart = ChartFactory.createPieChart(
            "", // Tiêu đề biểu đồ (ở đây là rỗng)
            pieDataset, // Dữ liệu cho biểu đồ
            true, // Hiển thị chú thích (legend)
            true, // Hiển thị các công cụ (tooltips)
            false // Không tạo URL cho các mục
        );
        // Gán biểu đồ vừa tạo cho chart1
    ...
    public Phieu getPhieuNhapSelect() {
        ...
        Phieu pn = PhieuNhapImpl.getInstance().selectById(tblModel.getValueAt(i_row, 1).toString());
        return pn;
    }
    ...
    public Phieu findPhieu() {
        ...
        ArrayList<Phieu> phieuAll = PhieuNhapImpl.getInstance().selectAllP();
        Phieu phieuk = null;
    ...
    public void filterThongKeSanPham() throws ParseException {
        ...
        if (jDateChooserFromPr.getDate() != null || jDateChooserToPr.getDate() != null) {
            ...
            thongKe = ThongKeImpl.getInstance().getThongKe(from, to);

        } else {
            thongKe = ThongKeImpl.getInstance().getThongKe();
        }
    ...
    public static void main(String[] args) {
        javax.swing.SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                ThongKeForm myFrame = new ThongKeForm();
